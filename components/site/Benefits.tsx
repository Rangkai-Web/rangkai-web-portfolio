...
export function Benefits() {
  ...
  const currentBenefits = benefits[language] || benefits.id
  ...
  useEffect(() => {
    if (!api || currentBenefits.length < 3) {
      return
    }
  ...
  }, [api, currentBenefits.length])

  return (
    <section id="keunggulan" className="w-full bg-white dark:bg-gray-900" aria-label={t('benefits.title')}>
      <div className="container mx-auto max-w-7xl px-8 md:px-24 text-center pb-32">
        <Badge className="text-orange-600 dark:text-orange-400 font-medium bg-orange-100 dark:bg-orange-900/30 mb-4 text-base">{t('benefits.badge')}</Badge>
        <h2 className="text-3xl lg:text-4xl text-gray-900 dark:text-white mb-8">{t('benefits.title')}</h2>
