    ...
    <section id="testimonials" className="relative" aria-label="Testimoni">
      ...
      <div className="relative container mx-auto max-w-7xl px-8 md:px-24 py-32">
        <div className="text-center mb-8">
          <Badge className="text-orange-600 dark:text-orange-400 font-medium bg-orange-100 dark:bg-orange-900/30 mb-4 text-base">Testimoni</Badge>
              ...
              {testimonials.map((t, index) => (
                <CarouselItem key={t.name} className="pb-10 px-2 md:px-4 basis:1/3 lg:basis-1/2 xl:basis-1/3">
                  <div className="bg-white/80 dark:bg-gray-800/90 rounded-2xl border border-white dark:border-gray-700 p-6 shadow-xl hover:-translate-y-2 transition-all duration-300 h-full flex justify-items-start items-center flex-col gap-4">
                    ...
                    <h3 className="font-medium text-2xl dark:text-white">{t.name}</h3>
                    <p className="text-gray-600 dark:text-gray-300 text-base text-center">{t.role}</p>
                    <div className="flex items-center space-x-1">
                      {[...Array(t.rating)].map((_, i) => (
                        <Star key={i} className="w-5 h-5 fill-orange-500 dark:fill-orange-400 text-orange-500 dark:text-orange-400" />
                      ))}
                    </div>
                    <p className="text-gray-600 dark:text-gray-300 text-base text-center">{t.content}</p>
                  </div>
                </CarouselItem>
              ))}
            </CarouselContent>
